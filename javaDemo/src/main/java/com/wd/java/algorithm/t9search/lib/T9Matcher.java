...
public final class T9Matcher {
	...
	public static T9MatchInfo matches(String t9Key, char t9Constraint) {
		...
		if (!isEmpty(t9Key)) {
			...
			if (index >= 0) {
				int begin = t9Key.substring(0, index).lastIndexOf(
						T9Utils.T9_KEYS_DIVIDER) + 1;
	...
	public static T9MatchInfo matches(String t9Key, String t9Constraint) {
		if (isEmpty(t9Key) || isEmpty(t9Constraint))
	...
	private static int matchesName(String t9Key, int begin, int end, int start,
			String t9Constraint, int cStart, BitSet bitSet) {
		int nextInitialCharIndex = start + 1;
		System.out.println("#3 begin="+begin+" end="+end+" start="+start);
		do {
			if (nextInitialCharIndex >= end
					|| T9Utils.isInitial(t9Key.charAt(nextInitialCharIndex))) {
				if (nextInitialCharIndex == end) {
					if (t9Key.regionMatches(start + 1, t9Constraint,
							cStart + 1, -1 + (t9Constraint.length() - cStart))) {
						bitSet.set(start - begin, (start - begin)
								+ t9Constraint.length() - cStart);
						return 1;
					} else {
						return 0;
					}
		...
		if (T9Utils.convertDigitToInitial(t9Constraint.charAt(cStart + 1)) == t9Key
				.charAt(nextInitialCharIndex)) {
			...
			int matchCount = matchesName(t9Key, begin, end,
					nextInitialCharIndex, t9Constraint, cStart + 1, bitSet);
			if (matchCount > 0) {
				bitSet.set(start - begin);
				return matchCount + 1;
			}
		...
		if (t9Constraint.length() - cStart <= spanLength) {
			if (t9Key.regionMatches(start + 1, t9Constraint, cStart + 1, -1
					+ (t9Constraint.length() - cStart))) {
				bitSet.set(start - begin,
		...
		if (T9Utils.convertDigitToInitial(t9Constraint.charAt(cStart
				+ spanLength)) == t9Key.charAt(nextInitialCharIndex)
				&& t9Key.regionMatches(start + 1, t9Constraint, cStart + 1,
						spanLength - 1)) {
			...
			int matchCount = matchesName(t9Key, begin, end,
					nextInitialCharIndex, t9Constraint, cStart + spanLength,
					bitSet);
			if (matchCount > 0) {
	...
	private static void matchesName(T9MatchInfo matchInfo, String t9Key,
			int start, int end, String t9Constraint) {
		System.out.println("#2 start="+start+" end="+end+" t9Constraint="+t9Constraint);
		if (end - start < t9Constraint.length())
			return;

		final int maxLength = 1 + (end - t9Constraint.length());
		final char first = T9Utils
				.convertDigitToInitial(t9Constraint.charAt(0));
		...
		while (true) {
			...
			int matchCount = matchesName(t9Key, start, end, index,
					t9Constraint, 0, bitSet);
			if (matchCount > 0) {
	...
	private static void setMatchResult(String t9Key, T9MatchInfo matchInfo,
			BitSet bitSet, int begin) {
		int wordCount = 0;
		int start = -1;
	...
	private static T9MatchInfo checkMatchInfo(T9MatchInfo matchInfo) {
		if (matchInfo.found()) {
	...
	public static T9MatchInfo matchesNumber(String phoneNumber,
			String constraint) {
		T9MatchInfo matchInfo = new T9MatchInfo();
		if (isEmpty(phoneNumber) || isEmpty(constraint))
			return matchInfo;
