...
public class FreeView extends ImageView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        maxWidth = WindowSizeUtil.getMaxWidth(context);
        // 此时减去状态栏高度,注意如果有状态栏,要减去状态栏,如下行得到的是可活动的高度
        maxHeight = WindowSizeUtil.getMaxHeight(context)
                - WindowSizeUtil.getStatusBarHeight(context);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (this.isEnabled()) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN: // 点击动作处理 每次点击时将拖动状态改为 false
                    // 并且记录下点击时的坐标 downX downY
                    isDrag = false;
                    downX = event.getX(); // 点击触屏时的x坐标 用于离开屏幕时的x坐标作计算
                    downY = event.getY(); // 点击触屏时的y坐标 用于离开屏幕时的y坐标作计算
                    break;
                case MotionEvent.ACTION_MOVE: // 滑动动作处理 记录离开屏幕时的 moveX moveY 用于计算距离
                    // 和 判断滑动事件和点击事件 并作出响应
                    ...
                    if (Math.abs(moveX) > 3 || Math.abs(moveY) > 3) {
                        ...
                        if (l < 0) { // left 小于 0 就是滑出边界 赋值为 0 ; right 右边的坐标就是自身宽度
                            // 如果可以划出边界 left right top bottom 最小值的绝对值
                            // 不能大于自身的宽高
                            l = 0;
                            r = l + width;
