...
public class SinSurfaceView extends SurfaceView implements Runnable {
    ...
    private void init() {
        ...
        /**
         * 通过holder去申请绘图表面的画布，surfaceview其实draw()或dispathDraw()都只是一块默认的黑色区域，
         * 并不是用作宿主 真正要做的事情由开发者自行绘制，绘制之前就是通过holder获取一块内存区域的画布，
        ...
        mSurfaceHolder.addCallback(new SurfaceHolder.Callback() {
             ...
             * 在updateWindow
             * （）创建宿主（activity的窗口）的绘图表面时会回调，虽然surfaceView是独立于一个线程但还是离不开宿主窗口，
             * 最后还是要粘贴到window中
             *
